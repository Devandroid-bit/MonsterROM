...
{
    local INIT_RC="$WORK_DIR/vendor/etc/init/init.exynos990.rc"
...
{
    LOG "- Disabling UFFD GC, virtual Perfetto relay, and RKP paths unsupported by the legacy Exynos vendor"
...
{
    LOG "- Disabling Bluetooth audio offload paths unsupported by the legacy Exynos vendor"
...
_DISABLE_UNSUPPORTED_OUI9_INIT_WRITES()
{
    ...
    _SED_DELETE_IF_EXISTS "$WORK_DIR/vendor/etc/init/init.exynos990.rc" \
...
{
    LOG "- Disabling bootchecker rescue-party loop for legacy Exynos boots"
    DELETE_FROM_WORK_DIR "system" "system/etc/init/bootchecker.rc"
}

_DISABLE_ZYGOTE_NEXT_BOOT()
{
    LOG "- Disabling zygote_next on legacy Exynos vendor stack"
...
_RELAX_VOLD_REBOOT_ON_FAILURE()
{
    ...
    if grep -q "^[[:space:]]*reboot_on_failure " "$VOLD_RC"; then
        LOG "- Removing vold reboot_on_failure on legacy Exynos vendor stack"
...
ADD_TO_WORK_DIR "platform/exynos990/patches/miscs" "vendor" "etc/ueventd.rc" 0 0 644 "u:object_r:vendor_configs_file:s0"
ADD_TO_WORK_DIR "platform/exynos990/patches/miscs" "vendor" "etc/bluetooth_audio_policy_configuration.xml" 0 0 644 "u:object_r:vendor_configs_file:s0"
ADD_TO_WORK_DIR "platform/exynos990/patches/miscs" "vendor" "etc/init/android.hardware.sensors@2.0-service-multihal.rc" 0 0 644 "u:object_r:vendor_configs_file:s0"
ADD_TO_WORK_DIR "platform/exynos990/patches/miscs" "vendor" "bin/monsterrom_wait_sensors_ready.sh" 0 2000 755 "u:object_r:vendor_file:s0"
ADD_TO_WORK_DIR "platform/exynos990/patches/miscs" "system" "system/etc/default-permissions/default-permissions-com.samsung.android.beaconmanager.xml" 0 0 644 "u:object_r:system_file:s0"
