...
run_erofs_binder() {
    ...
    if [ -f "$binder" ]; then
        log_msg "running EROFS binder from service"
...
log_msg "service stage reached"
run_erofs_binder

(
    sleep 20
    log_msg "service delayed property guard reached"
    run_erofs_binder
    ...
    if command -v cmd >/dev/null 2>&1; then
        cmd device_config put runtime_native_boot enable_uffd_gc false 2>/dev/null || true
    fi
) &
