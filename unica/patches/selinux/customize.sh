...
HWSERVICE_DUPLICATES+="
vendor.display.config::IDisplayConfig
"
...
# halves of the target's policy mapping floor; retaining only system is not
...
for e in $SERVICE_DUPLICATES; do
    if grep -F -q "$e" "$WORK_DIR/vendor/etc/selinux/vendor_service_contexts" && \
            grep -v -E "^[[:space:]]*#" "$WORK_DIR/$(GET_SYSTEM_EXT)/etc/selinux/system_ext_service_contexts" | grep -F -q "$e"; then
        PATCHED=true
        LOG "- \"$e\" SELinux duplicate service found after vendor context restoration. Removing from system_ext"
        sed -i "s|^$e|#SEC_DUPLICATE: $e|g" "$WORK_DIR/$(GET_SYSTEM_EXT)/etc/selinux/system_ext_service_contexts"
    fi
done

for e in $HWSERVICE_DUPLICATES; do
    if grep -F -q "$e" "$WORK_DIR/vendor/etc/selinux/vendor_hwservice_contexts" && \
            grep -v -E "^[[:space:]]*#" "$WORK_DIR/$(GET_SYSTEM_EXT)/etc/selinux/system_ext_hwservice_contexts" | grep -F -q "$e"; then
        PATCHED=true
        LOG "- \"$e\" SELinux duplicate hwservice found after vendor context restoration. Removing from system_ext"
        sed -i "s|^$e|#SEC_DUPLICATE: $e|g" "$WORK_DIR/$(GET_SYSTEM_EXT)/etc/selinux/system_ext_hwservice_contexts"
    fi
done
...
_APPEND_GENFSCON()
{
    ...
    # genfscon targets must be concrete types declared by the platform CIL;
    # a versioned type declaration elsewhere is not enough here.
    if ! grep -q -F "(type $TYPE)" "$FILE"; then
        LOGW "SELinux type not found for genfscon $FS $FS_PATH: $TYPE"
        return 0
    fi

    RULE="(genfscon $FS $FS_PATH (u object_r $TYPE ((s0) (s0))))"
    if ! grep -q -F "$RULE" "$FILE"; then
        PATCHED=true
        LOG "- Adding SELinux genfscon: $FS $FS_PATH -> $TYPE"
        printf "%s\n" "$RULE" >> "$FILE"
    fi
}
...
if _CIL_SYMBOL_EXISTS "init" && grep -q -F "(type sysfs_nfc_power_writable)" "$PLAT_SEPOLICY"; then
    _APPEND_CIL_RULE "$PLAT_SEPOLICY" "(allow init sysfs_nfc_power_writable (file (open read getattr write)))"
fi
if _CIL_SYMBOL_EXISTS "vendor_init" && grep -q -F "(type sysfs_nfc_power_writable)" "$PLAT_SEPOLICY"; then
...
unset ENTRIES DUPLICATES SERVICE_DUPLICATES HWSERVICE_DUPLICATES CIL_NAME PATCHED SYSTEM_EXT_DIR SELINUX_DIRS VENDOR_API_LIST MAPPING_FILE ENGMODE_SERVICE_TYPE ENGMODE_SERVICE_NAME SYSTEM_EXT_SEPOLICY VENDOR_SEPOLICY PLAT_SEPOLICY SYSTEM_EXT_SEAPP SYSTEM_EXT_SEAPP_RULE
unset -f GET_SYSTEM_EXT RESTORE_TARGET_MAPPING _CLEAN_UNDECLARED_MAPPING_ATTRS _TYPE_EXISTS _CIL_SYMBOL_EXISTS _APPEND_CONTEXT _APPEND_SEAPP_CONTEXT _SELECT_CIL_TYPE _APPEND_PROP_ALLOW _APPEND_PROP_READ _APPEND_CIL_RULE _APPEND_GENFSCON _APPEND_SERVICE_FIND _APPEND_HWSERVICE_FIND _APPEND_PROCESS_ALLOW
